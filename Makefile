...
all:
	$(CXX) $(CXXFLAGS) $(LDFLAGS) $(EXTRA_FLAGS) $(SRC) -o hyprexpo.so `pkg-config --cflags pixman-1 libdrm hyprland hyprgraphics pangocairo libinput libudev wayland-server xkbcommon`
clean:
	rm -f ./hyprexpo.so
